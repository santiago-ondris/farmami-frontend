...
const ProveedorEvaluacionFormPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchEvaluacion = async () => {
      try {
        const { data } = await api.get(`/api/evaluaciones-proveedores/${evaluacionId}`);
        setFormData({
          ...buildInitialState(),
          ...data,
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (isEditing) {
        await api.patch(`/api/evaluaciones-proveedores/${evaluacionId}`, payload);
        toast.success('Evaluacion actualizada');
      } else {
        await api.post(`/api/proveedores/${id}/evaluaciones`, payload);
    ...
    <div className="mx-auto max-w-4xl space-y-6 font-['var(--font-body)']">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-['var(--font-heading)'] text-3xl font-bold text-[var(--color-primary)]">
            {isEditing ? 'Editar evaluacion de proveedor' : 'Nueva evaluacion de proveedor'}
          </h1>
          <p className="text-sm text-gray-500">Evaluacion APTO/NO APTO sobre documentacion habilitante.</p>
        </div>
        <Link to={`/proveedores/${id}`} className="text-sm text-gray-500 hover:underline">Volver</Link>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 rounded-lg border border-gray-100 bg-white p-6 shadow-sm">
        ...
        <div className="flex justify-end gap-2 border-t border-gray-100 pt-4">
          <Link to={`/proveedores/${id}`} className="rounded border border-gray-300 px-6 py-2 font-semibold hover:bg-gray-50">Cancelar</Link>
...
export default ProveedorEvaluacionFormPage;
